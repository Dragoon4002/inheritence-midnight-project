/**
 * Standalone deploy script for the inheritance contract.
 *
 * Usage:
 *   npm run build && npm run deploy
 *
 * Prompts for a hex seed (or generates a new one), builds a wallet,
 * waits for sync + funds + dust, deploys the inheritance contract,
...
import { Contract } from './managed/inheritance/contract/index.js';
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'inheritance');
...
// Types
// ---------------------------------------------------------------------------
type InheritancePrivateState = Record<string, never>;
type InheritanceCircuits = ImpureCircuitId<Contract<InheritancePrivateState>>;
const InheritancePrivateStateId = 'inheritancePrivateState' as const;
...
async function main() {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║        Inheritance Contract Deploy Script               ║');
  ...
  const zkConfigProvider = new NodeZkConfigProvider<InheritanceCircuits>(zkConfigPath);

  const providers = {
    privateStateProvider: levelPrivateStateProvider<typeof InheritancePrivateStateId>({
      privateStateStoreName: 'inheritance-private-state',
  ...
  // --- Compile contract ---
  const inheritanceCompiledContract = CompiledContract.make('inheritance', Contract).pipe(
    CompiledContract.withVacantWitnesses,
    CompiledContract.withCompiledFileAssets(zkConfigPath),
  );

  // --- Deploy ---
  const contract = await withStatus('Deploying inheritance contract (this may take a few minutes)', async () => {
    return deployContract(providers, {
      compiledContract: inheritanceCompiledContract,
      privateStateId: InheritancePrivateStateId,
      initialPrivateState: {},
    });
  });
